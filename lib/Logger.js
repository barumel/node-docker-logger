...
class Logger {
  ...
  async start() {
    ...
    console.log(`${new Date()}: Logger started at ${new Date()}...`);
    ...
    this.tasks = info.map((container) => {
      ...
      console.log(`${new Date()}: Create task for container with name ${container.Name}...`);

      if (!fs.pathExistsSync(config.dir)) {
        console.log(`${new Date()}: Logfile directory ${config.dir} does not exists. Create...`);
      ...
      task.on('error', (err) => {
        console.log(`${new Date()}: Log task for container ${container.Name} encountered and error: ${err}`, err);
      });

      task.on('end', (code) => {
        console.log(`${new Date()}: Log task for container ${container.Name} ended with exit code ${code}!`);
        console.log(`${new Date()}: Try to restart task for container ${container.Name}...`)
        task.start();
      });
